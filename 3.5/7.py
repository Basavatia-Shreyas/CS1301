...
def average_word_length(my_string):
    ...
    try:
        for i in range(len(my_string)):
            #print(i)
            if (my_string[i] in alphabet):
                letters += 1
            try:
                if not (my_string[i] in alphabet) and (my_string[i + 1] in alphabet):
                    word_count += 1
            except:
                pass
        try:
            if(my_string[0] in alphabet):
                word_count += 1
            #print(letters, word_count)
