...
def to_metric(a_string):
    if "gallons" in a_string:
        return round(float(a_string.split(" ")[0]) * 3785.41, 2)
    elif "quarts" in a_string:
        return round(float(a_string.split(" ")[0]) * 946.35, 2)
    elif "pints" in a_string:
        return round(float(a_string.split(" ")[0]) * 473.18, 2)
    elif "cups" in a_string:
        return round(float(a_string.split(" ")[0]) * 240, 2)
    elif "ounces" in a_string:
        return round(float(a_string.split(" ")[0]) * 29.57, 2)
    elif "tablespoons" in a_string:
        return round(float(a_string.split(" ")[0]) * 14.79, 2)
    elif "teaspoons" in a_string:
        return round(float(a_string.split(" ")[0]) * 4.93, 2)
