...
def encrypt(message):
    ...
    for i in range(0, splits * 2, 2):
        if new_message[i] == new_message[i+1]:
            pairs.append(new_message[i] + "X")
        else:
            pairs.append(new_message[i] + new_message[i+1])
    #print(pairs)
    ...
    for i in pairs:
        ...
        if first[0] != second[0] and first[1] != second[1]:
            temp += CIPHER[first[0]][second[1]]
            temp += CIPHER[second[0]][first[1]]
            #print(i, temp)
            crypted.append(temp)
        #Column case
        elif first[0] != second[0] and first[1] == second[1]:
            temp += CIPHER[(first[0] + 1) % 5][first[1]]
            temp += CIPHER[(second[0] + 1) % 5][second[1]]
            #print(i, temp)
            crypted.append(temp)
        #Row Case
        elif first[0] == second[0] and first[1] != second[1]:
            temp += CIPHER[first[0]][(first[1] + 1) % 5]
            temp += CIPHER[second[0]][(second[1] + 1) % 5]
            #print(i, temp)
            crypted.append(temp)
