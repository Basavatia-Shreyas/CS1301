...
def check_winner(a_list):
    for i in range(6):
        next_four_vert = [None, None, None, None]
        next_four_hor = [None, None, None, None]
        next_four_diag = [None, None, None, None]
        for j in range(7):
            if i < 3:
                next_four_vert = [a_list[i][j], a_list[i+1][j], a_list[i+2][j], a_list[i+3][j]]
            if j < 4:
                next_four_hor = [a_list[i][j], a_list[i][j+1], a_list[i][j+2], a_list[i][j+3]]
                
            if j < 4 and i < 3:  
                next_four_diag = [a_list[i][j], a_list[i+1][j+1], a_list[i+2][j+2], a_list[i+3][j+3]]
            
            
            #print(i, j, next_four_vert, next_four_hor)
            if (not "X" in next_four_vert and not None in next_four_vert):
                return "O"
            if (not ("O" in next_four_vert) and not (None in next_four_vert)):
                return "X"
            if (not ("X" in next_four_hor) and not (None in next_four_hor)):
                return "O"
            if (not ("O" in next_four_hor) and not (None in next_four_hor)):
                return "X"
            if (not ("X" in next_four_diag) and not (None in next_four_diag)):
                return "O"
            if (not ("O" in next_four_diag) and not (None in next_four_diag)):
                return "X"
...
nowins =(("X" , "X" , None, None, None, None, None),
         ...
         ("X" , "X" , "X" , "O" , "X" , "X" , "O" ),
         ("X" , "O" , "O" , "X" , "O" , "X" , "O" ))
another_x_wins = (('X', 'X', None, None, None, None, None), ('O', 'O', None, None, None, None, None), ('O', 'X', 'O', 'O', None, 'O', 'O'), ('O', 'X', 'X', 'X', 'X', 'X', 'X'), ('X', 'X', 'X', 'O', 'X', 'X', 'O'), ('X', 'O', 'O', 'X', 'O', 'X', 'O'))
         
print(check_winner(xwins))
print(check_winner(owins))
print(check_winner(nowins))
print(check_winner(another_x_wins))
