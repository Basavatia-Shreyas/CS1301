...
#(look in the d-rop-down in the top left). If we were to call
...
def generate_name(filename, name):
    first_name_letter = ord(name[0]) - 65
    last_name_letter = ord(name.split(" ")[1][0]) -65
    data = open(filename, "r")
    heronames = data.readlines()
    for i in range(len(heronames)-1):
        heronames[i] = heronames[i][:-1]
    return heronames[first_name_letter] + " " + heronames[last_name_letter + 26]
    
    
    print(chr(last_name_letter), heronames)
